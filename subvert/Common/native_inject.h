...
namespace native
{
	...
	static HANDLE WINAPI ShellcodeBegin(PTHREAD_DATA parameter) {
		if (parameter->fnRtlInitUnicodeString != nullptr&&parameter->fnLdrLoadDll != nullptr) {
			...
			if (parameter->fnLdrSetDllManifestProber)
			{
	...
	private:
		bool ProcessInternalExecute(PTHREAD_DATA parameter, DWORD process_id) {
			...
			if (!NT_SUCCESS(ZwOpenProcess(&hProcess, PROCESS_ALL_ACCESS, &oa, &cid))) {
				DBG_PRINT(_T("ZwOpenProcess failed\r\n"));
				return false;
			}
			...
			if (!data || !code) {
				NtClose(hProcess);
				DBG_PRINT(_T("VirtualAllocEx failed\r\n"));
				return false;
			}
			...
			if (!NT_SUCCESS(RtlCreateUserThread(hProcess, NULL, FALSE, 0, 0, 0, code, data, &hThread, NULL))) {
				NtClose(hProcess);
				DBG_PRINT(_T("RtlCreateUserThread failed\r\n"));
				return false;
			}
	...
	public:
		bool inject_dll(HANDLE ProcessId, wchar_t *dll_path)
		{
			...
			if (!pNtDllBase)
			{
				DBG_PRINT(_T("GetModuleHandle failed\r\n"));
				return false;
			}
			auto pfnLdrLoadDll = GetProcAddress(pNtDllBase, "LdrLoadDll");
			if (!pfnLdrLoadDll)
			{
				DBG_PRINT(_T("GetProcAddress failed\r\n"));
				return false;
			}
			DBG_PRINT(_T("LdrLoadDll = %p\r\n"), pfnLdrLoadDll);

			if (!handle ||
				handle==INVALID_HANDLE_VALUE)
			{
				DBG_PRINT(_T("OpenProcess failed\r\n"));
				return false;
			}
			...
			if (!pBuffer)
			{
				DBG_PRINT(_T("AllocateInjectMemory failed\r\n"));
				return false;
			}
			...
			*(ULONGLONG*)((PUCHAR)Buffer.code + 32+1) = (ULONGLONG)pfnLdrLoadDll;
			...
			if (!NT_SUCCESS(ns))
			{
				DBG_PRINT(_T("ZwWriteVirtualMemory failed\r\n"));
				return false;
			}
			HANDLE hThread = nullptr;
